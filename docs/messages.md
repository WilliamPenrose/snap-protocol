...
The **responder** generates and assigns `contextId`. Requesters do not create or suggest context IDs.

- When a requester sends `message/send` without a `taskId`, the responder creates a new task. The responder decides whether to assign it to an existing context or create a new one.
- When a requester continues a task (with `taskId`), the task's existing `contextId` is preserved.
...
4. Responders SHOULD return `contextId` in every task response so requesters can reference it for diagnostics and logging.
...
|---------------------|-----------|-------------------------------|
| Responder-generated | Responder | Each request creates new task |
| Requester-generated | Requester | Same `taskId` = same task     |

**v0.1**: Task IDs are generated by the responder.

To achieve idempotent task creation, requesters SHOULD:
...
```json
{
  "method": "message/send",
  "payload": {
    "idempotencyKey": "requester-generated-uuid",
    "message": { ... }
  }
}
```
...
This tells the requester that the request was not processed again — the response is from the original execution. Without this flag, a requester retrying after a network timeout cannot distinguish "first request succeeded" from "retry succeeded".
